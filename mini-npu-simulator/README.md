...
- **MAC 연산**: `MacUnit.compute()`(`mac_unit.py`)가 반복문 두 겹으로 패턴과 필터를 같은 위치끼리 곱해 모두 더합니다. 값이 큰 칸끼리 같은 위치에 있을 때만 곱이 커지므로, 점수가 높을수록 두 격자의 활성 위치가 많이 겹친다는 뜻 — 곧 더 유사하다는 의미입니다. NumPy 등 벡터화 라이브러리는 사용하지 않았습니다.
- **라벨 정규화**: `LabelNormalizer`(`labels.py`)가 `expected`의 `'+'`/`'x'`뿐 아니라 필터 딕셔너리의 키(`cross`/`x`)도 같은 `normalize()`로 정규화해서 찾습니다(`find_filter()`). 그래서 필터 키의 대소문자가 달라도(`Cross`/`X` 등) 정상적으로 필터를 찾습니다. 이 로직을 별도 클래스로 뗀 이유는 전처리와 판정 로직의 결합도를 낮추고, 규칙이 바뀌거나 라벨이 늘어나도 이 클래스 하나만 고치면 되게 하기 위해서입니다.
- **동점(부동소수점) 처리 정책**: `Judge.decide()`(`judge.py`)가 `abs(score_a - score_b) < 1e-9`이면 `UNDECIDED`를 반환합니다. 점수가 다른 경로(단일 곱셈 vs 여러 번의 덧셈 누적)로 계산될 경우 수학적으로 같은 값이라도 이진 부동소수점 표현 때문에 마지막 자리에서 미세하게 어긋날 수 있기 때문입니다. 이 규칙은 모드 1·모드 2가 동일하게 재사용하지만, 모드 1은 비교할 정답이 없어 "판정 불가"로만 알리고 모드 2는 `UNDECIDED`를 정답 라벨(`Cross`/`X`)과 절대 겹치지 않는 값으로 써서 동점이 우연히 PASS로 잘못 집계되는 걸 막습니다.
...
- **성능 분석**: `PerformanceBenchmark`(`performance.py`)가 크기별로 `PatternGenerator.cross(n)`(`pattern_generator.py`)을 직접 생성해 벤치마크합니다. MAC 연산 시간은 값의 내용이 아니라 크기(N)에만 영향을 받으므로, `data.json`에 해당 크기의 패턴이 있는지와 무관하게 항상 동일한 방식으로 3×3/5×5/13×13/25×25를 측정할 수 있습니다. `perf_counter`는 `MacUnit.compute()` 호출 바로 앞뒤에만 둬서, 입력 수집이나 파일 읽기 같은 I/O 시간이 측정 구간에 섞이지 않게 했습니다.
...
지금 구현된 내용이 아니라 "이렇게 확장한다면"에 대한 답입니다.

- **새 라벨 추가**: 데이터(`LabelNormalizer`에 라벨 추가) → 파서(2개 비교용 판정 로직을 N-way로 일반화) → 로직(출력·테스트 반영) 순서로, 가장 위험한 판정 로직부터 안전하게 바꿉니다.
- **대규모 패턴(예: 1000×1000) 처리**: 연산량이 25×25 대비 약 1,600배로 늘어나고, 지금은 파이썬 반복문 단일 스레드라 병목이 연산 유닛 처리량에 생깁니다. 2차원 리스트를 1차원으로 펴서 인덱싱 비용을 줄이는 게 1차 개선이고, 근본적으로는 NPU처럼 곱셈-누적 유닛을 병렬 배치해야 합니다(이땐 병목이 메모리 대역폭·최종 합산 단계로 이동합니다).
